...
void MyEvoAlgo::configure(farsa::ConfigurationParameters& params, QString prefix)
{
	...
	m_funct = farsa::ConfigurationHelper::getInt(params, prefix + "funct", m_funct);
	m_n = farsa::ConfigurationHelper::getInt(params, prefix + "n", m_n);
}
...
void MyEvoAlgo::genoToPheno(const QVector<int> geno, QVector<double>& pheno)
{
	// Convert integers in the range [0,255] into floating points in the range [-5.0,5.0]
	const double wrange = 5.0;
	if (pheno.size() != m_n)
		pheno.resize(m_n);
	for (int i = 0; i < m_n; i++)
	{
		pheno[i] = wrange - ((double)geno[i] / 255.0) * wrange * 2.0;
	}
}

double MyEvoAlgo::rastrigin(QVector<double> x)
{
	double f_out = 10.0 * m_n;
	for (int i = 0; i < m_n; i++)
		f_out += ((x[i] * x[i]) - (10.0 * cos(2.0 * PI_GRECO * x[i])));
	return f_out;
}

double MyEvoAlgo::rosenbrock(QVector<double> x)
{
	double f_out = 0.0;
...
double MyEvoAlgo::sphere(QVector<double> x)
{
	double f_out = 0.0;
	for (int i = 0; i < m_n; i++)
		f_out += (x[i] * x[i]);
...
double MyEvoAlgo::evaluate(QVector<double> x)
{
	if (m_funct == 0)
		return rastrigin(x);
	else if (m_funct == 1)
...
double MyEvoAlgo::annealing(const QVector<int> original, double fitness, double actualFitness, QVector<int>& novel, int& steps, bool& solved)
{
	...
	QVector<int> evalInd(m_n);
	QVector<double> candidate(m_n);
	int annGn = 0;
	double fit = 0.0;
...
void MyEvoAlgo::computeFStat()
{
	// Fitness is inverted (lower is better)
	fmax = 9999999999.0;
	fmin = -9999999999.0;
	faverage = 0.0;
...
void MyEvoAlgo::evolve()
{
	...
	QVector<int> evalInd(m_n); // Vector to be used to avoid overwritings!!
	// replications
        	...
        	//for(gn=startGeneration;gn<nogenerations;gn++) 
		{
			...
			for(id = 0; id < popSize; id++) 
			{
				...
				if ((gn == startGeneration) || (m_fitNoise > 0.0))
				{
					...
					ntfitness[id] = 0.0;
					// evaluate the parent
					genoToPheno(m_pop[id], candidate);
					fit = evaluate(candidate);
					// update its fitness
					tfitness[id] = fit;
					ntfitness[id] = 1;
				...
				// evaluate the child
				steps = exp->getNSteps();
				genoToPheno(m_pop[popSize + id], candidate);
				fit = evaluate(candidate);
				// update its fitness
...
void MyEvoAlgo::evolveHC()
{
	...
	// Individual to be evaluated
	QVector<double> candidate(m_n); // Vector to be used to avoid overwritings!!
	// replications
    	for(rp = 0; rp < nreplications; rp++) 
	{
		startGeneration = 0;
