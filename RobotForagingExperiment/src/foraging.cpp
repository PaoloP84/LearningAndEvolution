...
void BasicForagingExperiment::initTrial(int trial)
{
	...
	Evonet* evonet = getResource<Evonet>("agent[0]:evonet");
	evonet->resetNet();
}
...
void BasicForagingExperiment::initStep(int /*step*/)
{
	ResourcesLocker locker(this);
    	...
    	// Getting the robot and its position in the arena
	RobotOnPlane* robot = getResource<RobotOnPlane>("agent[0]:robot");
	m_robotPos = robot->position();
}
	...
	if (n != 0)
	{
		for (int i = 0; i < n; i++) 
		{
			if ( collidedObjects[i]->type() == farsa::PhyObject2DWrapper::Wall ) 
			{
				...
				// Set robot previous position
				robot->setPosition(arena->getPlane(), m_robotPos.x, m_robotPos.y);
			...
			else if ( collidedObjects[i]->type() == farsa::PhyObject2DWrapper::BigCylinder ) 
			{
				...
				// Update fitness
				trialFitnessValue++;
				// Place the food item in a different location of the arena
				placeFood(f);
			}
		}
	}
